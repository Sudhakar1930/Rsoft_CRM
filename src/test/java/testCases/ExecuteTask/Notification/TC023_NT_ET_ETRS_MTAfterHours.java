...
public class TC023_NT_ET_ETRS_MTAfterHours extends BaseClass{
	@BeforeTest()
	public void testName() {
		System.out.println("Before Test Method");
		test = extent.createTest("TC023_NT_ET_ETRS_MTAfterHours");
	}
	
	@Test
	public void testExecuteTask_NT_MTAfterHours () throws Exception {
		node = test.createNode("NT_ExecuteTask_MTAfterMinutes");
		UtilityCustomFunctions.logWriteConsole("******starting TC023_NT_ET_ETRS_MTAfterHours ****");
...
//		String sPath=".\\testData\\ExecuteTask\\Notification\\ETRS\\ET_NT_ETRS_MTAfterHours_Live" + ".xlsx" ;
		String sPath=".\\testData\\ExecuteTask\\Notification\\ETRS\\ET_NT_ETRS_MTAfterHours_Test" + ".xlsx" ;
...
//			objHP.clickAvatar();
		...
		Thread.sleep(2000);
		objCRMRs.fAddValuestoETNotification("Test","//ExecuteTask//Notification//ETRS//ET_NT_ETRS_MTAfterHours_","Sheet1",false);
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
		Thread.sleep(2000);
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRS//ET_NT_ETRS_MTAfterHours_","Sheet1","@Add New", node);
		...
		if(isEntity==false) {
			objBase.freport(" Actual EntityId: "+sCurrModRecId + " not created in DB @Add New Record " , "pass",node);
		...
		objCRMRs.fAddValuestoETNotification("Test","//ExecuteTask//Notification//ETRS//ET_NT_ETRS_MTAfterHours_","Sheet2",false);
		...
		sCurrModRecId = objCRMRs.getModuleRecordId();
		Thread.sleep(2000);
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRS//ET_NT_ETRS_MTAfterHours_","Sheet2","@Summary Add New", node);
		objHP.clickLogoutCRM();
		isEntity = objPAP.IsEntityInQueue(sMySqlUrl, sMySqlUid, sMySqlPwd, "rsoft_workflowtask_queue", sCurrModRecId);
		if(isEntity==false) {
			objBase.freport(" Actual EntityId: "+sCurrModRecId + " not created in DB after @Summary add Record" , "pass",node);
		...
		objCRMRs.fAddValuestoETNotification("Test","//ExecuteTask//Notification//ETRS//ET_NT_ETRS_MTAfterHours_","Sheet3",true);
		objDVP.fSetToggleHeader(true);
		objDVP.fSetDetailVew(false);
		objSVP.fWaitTillControlVisible();
		driver.navigate().refresh();
		Thread.sleep(3000);
		sCurrModRecId = objCRMRs.getModuleRecordId();
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRS//ET_NT_ETRS_MTAfterHours_","Sheet3","@Duplicate Add New", node);
		
		Thread.sleep(2000);
		objHP.clickLogoutCRM();
		isEntity = objPAP.IsEntityInQueue(sMySqlUrl, sMySqlUid, sMySqlPwd, "rsoft_workflowtask_queue", sCurrModRecId);
		if(isEntity==false) {
			objBase.freport(" Actual EntityId: "+sCurrModRecId + " not created in DB after @Duplicate add Record" , "pass",node);
		...
		Thread.sleep(3000);
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRS//ET_NT_ETRS_MTAfterHours_","Sheet4","Edit & Save Record", node);
		sCurrModRecId = objCRMRs.getModuleRecordId();
