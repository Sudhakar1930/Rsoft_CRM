...
public class TC041_NT_ET_ETRM_MTAfterHours extends BaseClass{
	@BeforeTest()
	public void testName() {
		System.out.println("Before Test Method");
		test = extent.createTest("TC041_NT_ET_ETRM_MTAfterHours");
	}
	@Test
	public void testET_NT_ETRM_MTAfterHours () throws Exception {
		node = test.createNode("NT_ExecuteTask_MTAfterHours");
		UtilityCustomFunctions.logWriteConsole("******starting TC041_NT_ET_ETRM_MTAfterHours ****");
...
//		String sPath=".\\testData\\ExecuteTask\\Notification\\ETRM\\ET_NT_ETRM_MTAfterHours_Live" + ".xlsx" ;
		String sPath=".\\testData\\ExecuteTask\\Notification\\ETRM\\ET_NT_ETRM_MTAfterHours_Test" + ".xlsx" ;
...
//			objHP.clickAvatar();
		...
		Thread.sleep(2000);
		objCRMRs.fAddValuestoETNotification("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_MTAfterHours_","Sheet1",false);
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
		Thread.sleep(2000);
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_MTAfterHours_","Sheet1","@Add New", node);
		...
		if(bFlag==false) {
		 objBase.freport("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Add New", "pass",node);
		 UtilityCustomFunctions.logWriteConsole("Passed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Add New");
		 BaseClass.sAssertinFn.assertEquals("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Add New","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Add New");
		}
		else {
			objBase.freport("Actual EntityId: "+sCurrModRecId + " Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Add New", "fail",node);
			UtilityCustomFunctions.logWriteConsole("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Add New");
			BaseClass.sAssertinFn.assertEquals("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Add New","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Add New");
		}
		...
		objCRMRs.fAddValuestoETNotification("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_MTAfterHours_","Sheet2",false);
		...
		Thread.sleep(3000);
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_MTAfterHours_","Sheet2","@Summary Add", node);
		Thread.sleep(2000);
		sCurrModRecId = objCRMRs.getModuleRecordId();
		...
		bFlag = objPAP.IsEntityInQueue(sMySqlUrl, sMySqlUid, sMySqlPwd,"rsoft_workflowtask_queue", sCurrModRecId);
		if(bFlag==false) {
		 objBase.freport("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Summary Add", "pass",node);
		 UtilityCustomFunctions.logWriteConsole("Passed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Summary Add");
		 BaseClass.sAssertinFn.assertEquals("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Summary Add","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Summary Add");
		}
		else {
			objBase.freport("Actual EntityId: "+sCurrModRecId + " Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Summary Add", "fail",node);
			UtilityCustomFunctions.logWriteConsole("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Summary Add");
			BaseClass.sAssertinFn.assertEquals("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Summary Add","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Summary Add");
		}
		...
		objCRMRs.fAddValuestoETNotification("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_MTAfterHours_","Sheet3",true);
		objDVP.fSetToggleHeader(true);
		objDVP.fSetDetailVew(false);
		objSVP.fWaitTillControlVisible();
		driver.navigate().refresh();
		Thread.sleep(3000);
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_MTAfterHours_","Sheet3","@Duplicate Record", node);
		sCurrModRecId = objCRMRs.getModuleRecordId();
		...
		bFlag = objPAP.IsEntityInQueue(sMySqlUrl, sMySqlUid, sMySqlPwd,"rsoft_workflowtask_queue", sCurrModRecId);
		if(bFlag==false) {
		 objBase.freport("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Duplicate Record", "pass",node);
		 UtilityCustomFunctions.logWriteConsole("Passed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Duplicate Record");
		 BaseClass.sAssertinFn.assertEquals("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Duplicate Record","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Duplicate Record");
		}
		else {
			objBase.freport("Actual EntityId: "+sCurrModRecId + " Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Duplicate Record", "fail",node);
			UtilityCustomFunctions.logWriteConsole("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Duplicate Record");
			BaseClass.sAssertinFn.assertEquals("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Duplicate Record","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for MTAfterHours Task @Duplicate Record");
		}
		...
		//**************************** Edit with New Data *****************************
			...
			objCRMRs.fAddValuestoETNotification("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_MTAfterHours_","Sheet4",true);
			objDVP.fSetToggleHeader(true);
			objDVP.fSetDetailVew(false);
			objSVP.fWaitTillControlVisible();
			driver.navigate().refresh();
			Thread.sleep(3000);
			UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
//			objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_MTAfterHours_","Sheet4","Edit & Save Record", node);
			sCurrModRecId = objCRMRs.getModuleRecordId();
			...
			sReturnValue = objPAP.check_ET_Notify_CT(sMySqlUrl, sMySqlUid, sMySqlPwd,"rsoft_workflowtask_queue",sCurrModRecId,node,"Edit & Save","H",sModifiedTime);
			...
			sReturnValue = objPAP.check_ET_Notify_CT(sMySqlUrl, sMySqlUid, sMySqlPwd,"rsoft_workflowtask_queue",sCurrModRecId,node,"@Single Line Edit","H",sModifiedTime);
