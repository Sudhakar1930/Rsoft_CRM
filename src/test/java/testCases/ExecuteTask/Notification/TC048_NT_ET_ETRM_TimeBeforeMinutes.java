...
public class TC048_NT_ET_ETRM_TimeBeforeMinutes extends BaseClass{
	@BeforeTest()
	public void testName() {
		System.out.println("Before Test Method");
		test = extent.createTest("TC048_NT_ET_ETRM_TimeBeforeMinutes");
	}
	@Test
	public void testExecuteTask_TimeBeforeMinutes() throws Exception {
		node = test.createNode("NT_ExecuteTask_TimeBeforeMinutes");
		UtilityCustomFunctions.logWriteConsole("******starting TC047_NT_ET_ETRM_TimeBeforeMinutes ****");
...
//		String sPath=".\\testData\\ExecuteTask\\Notification\\ETRM\\ET_NT_ETRM_TimeBeforeMinutes_Live" + ".xlsx" ;
		String sPath=".\\testData\\ExecuteTask\\Notification\\ETRM\\ET_NT_ETRM_TimeBeforeMinutes_Test" + ".xlsx" ;
...
//			objHP.clickAvatar();
		...
		Thread.sleep(2000);
		objCRMRs.fAddValuestoETBySpecificValues("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_TimeBeforeMinutes_","Sheet1",false,"sTime");
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
		Thread.sleep(2000);
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_TimeBeforeMinutes_","Sheet1","@Add New", node);
		...
		if(bFlag==false) {
		 objBase.freport("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Add New", "pass",node);
		 UtilityCustomFunctions.logWriteConsole("Passed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Add New");
		 BaseClass.sAssertinFn.assertEquals("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Add New","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Add New");
		}
		else {
			objBase.freport("Actual EntityId: "+sCurrModRecId + " Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Add New", "fail",node);
			UtilityCustomFunctions.logWriteConsole("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Add New");
			BaseClass.sAssertinFn.assertEquals("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Add New","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Add New");
		}
		...
		objCRMRs.fAddValuestoETBySpecificValues("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_TimeBeforeMinutes_","Sheet2",false,"sTime");
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
		Thread.sleep(2000);
		...
		bFlag = objPAP.IsEntityInQueue(sMySqlUrl, sMySqlUid, sMySqlPwd,"rsoft_workflowtask_queue", sCurrModRecId);
		if(bFlag==false) {
		 objBase.freport("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Summary Add", "pass",node);
		 UtilityCustomFunctions.logWriteConsole("Passed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Summary Add");
		 BaseClass.sAssertinFn.assertEquals("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Summary Add","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Summary Add");
		}
		else {
			objBase.freport("Actual EntityId: "+sCurrModRecId + " Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Summary Add", "fail",node);
			UtilityCustomFunctions.logWriteConsole("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Summary Add");
			BaseClass.sAssertinFn.assertEquals("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Summary Add","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Summary Add");
		}
		...
		objCRMRs.fAddValuestoETBySpecificValues("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_TimeBeforeMinutes_","Sheet3",true,"sTime");
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
		Thread.sleep(2000);
		...
		bFlag = objPAP.IsEntityInQueue(sMySqlUrl, sMySqlUid, sMySqlPwd,"rsoft_workflowtask_queue", sCurrModRecId);
		if(bFlag==false) {
		 objBase.freport("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Duplicate Record", "pass",node);
		 UtilityCustomFunctions.logWriteConsole("Passed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Duplicate Record");
		 BaseClass.sAssertinFn.assertEquals("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Duplicate Record","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Duplicate Record");
		}
		else {
			objBase.freport("Actual EntityId: "+sCurrModRecId + " Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Duplicate Record", "fail",node);
			UtilityCustomFunctions.logWriteConsole("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Duplicate Record");
			BaseClass.sAssertinFn.assertEquals("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Duplicate Record","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for TimeBeforeMinutes Task @Duplicate Record");
		}
		...
		objCMD.clickEdit();
		Thread.sleep(3000);
		objCRMRs.fAddValuestoETBySpecificValues("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_TimeBeforeMinutes_","Sheet4",true,"sTime");
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
		Thread.sleep(2000);
		sCurrModRecId = objCRMRs.getModuleRecordId();
		Thread.sleep(2000);
		...
		int iHours = Integer.parseInt(sEDHours) - 1;
		...
		sAfterMinutes ="";
		iHours=0;
		iHours = Integer.parseInt(sEDHours) - 1;
		if(sAMorPM.equalsIgnoreCase("AM")){
			sAfterMinutes = "0" + String.valueOf(iHours) + ":15:00";
		}
		else {
		...
		if(sConfigDateTime.contains(sAfterMinutes)) {
			...
			BaseClass.sAssertinFn.assertEquals("ET Time after Minutes With Configured Time: " + sConfigDateTime + "Expected Time: "+ sAfterMinutes + " EntityId: "+sCurrModRecId + "@Single Line Edit","ET Time Before Minutes With Configured Time: " + sConfigDateTime + "Expected Time: "+ sAfterMinutes + " EntityId: "+sCurrModRecId + "@Single Line Edit");
		}else {
