...
public class TC044_NT_ET_ETRM_DateBeforeDays extends BaseClass {
	@BeforeTest()
	public void testName() {
		System.out.println("Before Test Method");
		test = extent.createTest("TC044_NT_ET_ETRM_DateBeforeDays");
	}
	@Test
	public void testET_NT_ETRM_DateBeforeDays () throws Exception {
		node = test.createNode("NT_ExecuteTask_DateBeforeDays");
		UtilityCustomFunctions.logWriteConsole("******starting TC044_NT_ET_ETRM_DateBeforeDays ****");
...
//		String sPath=".\\testData\\ExecuteTask\\Notification\\ETRM\\ET_NT_ETRM_DateBeforeDays_Live" + ".xlsx" ;
		String sPath=".\\testData\\ExecuteTask\\Notification\\ETRM\\ET_NT_ETRM_DateBeforeDays_Test" + ".xlsx" ;
...
//			objHP.clickAvatar();
		...
		if(bFlag==false) {
		 objBase.freport("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Add New", "pass",node);
		 UtilityCustomFunctions.logWriteConsole("Passed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Add New");
		 BaseClass.sAssertinFn.assertEquals("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Add New","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Add New");
		}
		else {
			objBase.freport("Actual EntityId: "+sCurrModRecId + " Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Add New", "fail",node);
			UtilityCustomFunctions.logWriteConsole("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Add New");
			BaseClass.sAssertinFn.assertEquals("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Add New","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Add New");
		}
		...
		objCRMRs.fAddValuestoETBySpecificValues("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_DateBeforeDays_","Sheet2",false,"sDate");
		...
		Thread.sleep(3000);
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_DateBeforeDays_","Sheet2","@Summary Add", node);
		Thread.sleep(2000);
		sCurrModRecId = objCRMRs.getModuleRecordId();
		...
		bFlag = objPAP.IsEntityInQueue(sMySqlUrl, sMySqlUid, sMySqlPwd,"rsoft_workflowtask_queue", sCurrModRecId);
		if(bFlag==false) {
		 objBase.freport("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Summary Add", "pass",node);
		 UtilityCustomFunctions.logWriteConsole("Passed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Summary Add");
		 BaseClass.sAssertinFn.assertEquals("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Summary Add","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Summary Add");
		}
		else {
			objBase.freport("Actual EntityId: "+sCurrModRecId + " Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Summary Add", "fail",node);
			UtilityCustomFunctions.logWriteConsole("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Summary Add");
			BaseClass.sAssertinFn.assertEquals("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Summary Add","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Summary Add");
		}
		...
		objCRMRs.fAddValuestoETBySpecificValues("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_DateBeforeDays_","Sheet3",true,"sDate");
		objDVP.fSetToggleHeader(true);
		objDVP.fSetDetailVew(false);
		objSVP.fWaitTillControlVisible();
		driver.navigate().refresh();
		Thread.sleep(3000);
		UtilityCustomFunctions.logWriteConsole("New Record added in: "+sExpModuleName);
//		objCRMRs.fVerifyETNotificationSummary("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_DateBeforeDays_","Sheet3","@Duplicate Record", node);
		sCurrModRecId = objCRMRs.getModuleRecordId();
		...
		bFlag = objPAP.IsEntityInQueue(sMySqlUrl, sMySqlUid, sMySqlPwd,"rsoft_workflowtask_queue", sCurrModRecId);
		if(bFlag==false) {
		 objBase.freport("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Duplicate Record", "pass",node);
		 UtilityCustomFunctions.logWriteConsole("Passed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Duplicate Record");
		 BaseClass.sAssertinFn.assertEquals("Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Duplicate Record","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Duplicate Record");
		}
		else {
			objBase.freport("Actual EntityId: "+sCurrModRecId + " Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Duplicate Record", "fail",node);
			UtilityCustomFunctions.logWriteConsole("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Duplicate Record");
			BaseClass.sAssertinFn.assertEquals("Failed: Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Duplicate Record","Actual EntityId: "+sCurrModRecId + " Not Created in DB for Every Time Record Modifies condition for DateBeforeDays Task @Duplicate Record");
		}
		...
		objCMD.clickEdit();
		Thread.sleep(3000);
		objCRMRs.fAddValuestoETBySpecificValues("Test","//ExecuteTask//Notification//ETRM//ET_NT_ETRM_DateBeforeDays_","Sheet4",true,"sDate");
		...
		System.out.println("Number of Days: " + days);
		 ...
		 if(days==1) {
			 objBase.freport("Date Selected: "+ d2 +" And Execution Start Date: "+ d1 +" Execute Task Notification Days before given Date is: "+days +" days and "+ "EntityId: "+sCurrModRecId + "@Edit & Save Record added" , "pass",node);
			 UtilityCustomFunctions.logWriteConsole("Date Selected: "+ d2 +" And Execution Start Date: "+ d1 +"Execute Task Notification Days before given Date is: "+days +" days and "+ "EntityId: "+sCurrModRecId + "@Edit & Save Record added"); 
			 BaseClass.sAssertinFn.assertEquals("Date Selected: "+ d2 +" And Execution Start Date: "+d1 +"Execute Task Notification Days before given Date is: "+days +" days and "+ "EntityId: "+sCurrModRecId + "@Edit & Save Record added","Date Selected: "+ d1 +" And Execution Start Date: "+d2 +"Execute Task Notification Days before given Date is: "+days +" days and "+ "EntityId: "+sCurrModRecId + "@Edit & Save Record added");
		 }else {
			 objBase.freport("Date Selected: "+ d2 +" And Execution Start Date: "+d1 +"Execute Task Notification Days before given Date is: "+days +" days and "+ "EntityId: "+sCurrModRecId + "@Edit & Save Record added" , "fail",node);
			 UtilityCustomFunctions.logWriteConsole("Failed: "+"Date Selected: "+ d2 +" And Execution Start Date: "+d1 +"Execute Task Notification Days before given Date is: "+days +" days and "+ "EntityId: "+sCurrModRecId + "@Edit & Save Record added"); 
			 BaseClass.sAssertinFn.assertEquals("Failed: "+ "Date Selected: "+ d2 +" And Execution Start Date: "+d1 +"Execute Task Notification Days before given Date is: "+days +" days and EntityId: "+sCurrModRecId + "@Edit & Save Record added","Execute Task Notification Days before given Date is: "+numberOfDays +" days and "+ "EntityId: "+sCurrModRecId + "@Edit & Save Record added"); 
		 }
		  objLP.clickPHPMyAdminLogout();
		  Thread.sleep(1000);
